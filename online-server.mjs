...
if(process.argv[1]&&pathToFileURL(fileURLToPath(import.meta.url)).href===pathToFileURL(process.argv[1]).href){
 const instance=await startOnlineServer({port:Number(process.env.PORT||8787),host:process.env.HOST||'127.0.0.1',allowedOrigins:[process.env.RENDER_EXTERNAL_URL,...(process.env.ALLOWED_ORIGINS||'').split(',')].filter(Boolean)});

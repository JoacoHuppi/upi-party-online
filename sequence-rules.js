...
export function createSequenceRound(seed){
 const random=seededRandom(seed),sequence=[Math.floor(random()*SEQUENCE_RULES.cells)],events=[];let bag=[];
 function nextCell(){if(!bag.length){bag=Array.from({length:SEQUENCE_RULES.cells},(_,i)=>i);for(let i=bag.length-1;i>0;i--){const j=Math.floor(random()*(i+1));[bag[i],bag[j]]=[bag[j],bag[i]];}}return bag.pop();}
 ...
 function advance(now){
  ...
  else if(phase==='cleared'&&now-phaseAt>=SEQUENCE_RULES.betweenMs){let cell=nextCell();if(cell===sequence.at(-1)&&bag.length){const swap=bag.pop();bag.push(cell);cell=swap;}sequence.push(cell);shown=cursor=0;phase='show';phaseAt=now;}

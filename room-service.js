...
export function createRoomService({now=()=>performance.now(),seed=()=>randomBytes(4).readUInt32LE(),graceMs=10000,countdownMs=3000}={}){
 ...
 function packet(r,p){return {protocol:1,transport:'websocket-v1',serverTime:now(),party:r.party,code:r.code,mode:r.mode,phase:r.phase,matchId:r.matchId,revision:r.revision,selfId:p.id,countdown:Math.max(0,r.startsAt-now()),game:r.engine?.snapshot()??null,result:r.result,timing:r.timing??null,session:sessionPacket(r),portals:r.party?ONLINE_PORTALS.map(zone=>({...zone,count:r.players.filter(x=>(x.bot||x.streams.size)&&onlinePortalAt(x.pose)===zone.id).length})):null,players:r.players.map(x=>({id:x.id,name:x.name,character:x.character,ready:x.ready,connected:x.bot||x.streams.size>0,wins:x.wins,pose:r.party?{...x.pose,seq:x.poseSeq}:null,grace:x.bot?null:!x.streams.size?Math.max(0,graceMs-(now()-x.offlineAt)):null}))};}
 ...
 function finish(r,winner,reason){if(r.phase==='result')return;const snapshot=r.engine?.snapshot?.()||{},scores=Array.isArray(snapshot.scores)?snapshot.scores:r.players.map(x=>({id:x.id,points:(snapshot.completed||0)*100}));for(const p of r.players){const source=scores.find(x=>x.id===p.id),points=source?.points??(snapshot.completed||0)*100,entry=r.session.get(p.id)||{points:0,games:0,wins:0};entry.points+=Number(points)||0;entry.games++;if(p.id===winner)entry.wins++;r.session.set(p.id,entry);}r.phase='result';r.result={winner,reason,matchId:r.matchId,scores};r.players.forEach(p=>{p.ready=false;if(p.id===winner)p.wins++;});r.updatedAt=now();publish(r);}
 function begin(r){r.matchId=randomUUID();r.phase='countdown';r.startsAt=now()+countdownMs;r.result=null;r.timing=null;r.receipts.clear();r.engine=null;r.botAt=0;r.botPressId=0;r.players.forEach(p=>{p.ready=false;p.pressId=0;p.actions.clear();});publish(r);}

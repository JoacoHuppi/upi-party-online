...
export function createRoomService({now=()=>performance.now(),seed=()=>randomBytes(4).readUInt32LE(),graceMs=10000,countdownMs=3000}={}){
 ...
 function packet(r,p){return {protocol:1,party:r.party,code:r.code,mode:r.mode,phase:r.phase,matchId:r.matchId,revision:r.revision,selfId:p.id,countdown:Math.max(0,r.startsAt-now()),game:r.engine?.snapshot()??null,result:r.result,portals:r.party?PARTY_PORTALS.map(zone=>({...zone,count:r.players.filter(x=>x.streams.size&&portalAt(x.pose)===zone.id).length})):null,players:r.players.map(x=>({id:x.id,name:x.name,character:x.character,ready:x.ready,connected:x.streams.size>0,wins:x.wins,pose:r.party?{...x.pose}:null,grace:!x.streams.size?Math.max(0,graceMs-(now()-x.offlineAt)):null}))};}
 ...
 function create({name,mode,character}){
  ...
  const p=newPlayer(name,character),r={code,party:mode==='party',mode:mode==='party'?null:mode,players:[p],phase:mode==='party'?'lobby':'waiting',matchId:randomUUID(),engine:null,startsAt:0,result:null,revision:0,updatedAt:now(),first:0};rooms.set(code,r);tokens.set(p.token,{r,p});return credentials(r,p);
 }
 function join({code,name,character}){const r=rooms.get(String(code).trim().toUpperCase());if(!r)fail('No existe esa sala.',404);if(r.players.length>=2)fail('La sala ya tiene dos jugadores.',409);if(r.phase==='result'){r.phase=r.party?'lobby':'waiting';r.engine=null;r.result=null;r.matchId=randomUUID();r.players.forEach(x=>{x.ready=false;x.actions.clear();});}if(!['waiting','lobby'].includes(r.phase))fail('La partida ya empezó.',409);const p=newPlayer(name,character);p.pose.x=1.2;r.players.push(p);tokens.set(p.token,{r,p});r.updatedAt=now();publish(r);return credentials(r,p);}
 ...
 function finish(r,winner,reason){if(r.phase==='result')return;r.phase='result';r.result={winner,reason,matchId:r.matchId};r.players.forEach(p=>{p.ready=false;if(p.id===winner)p.wins++;});r.updatedAt=now();publish(r);}
 ...
 function action(token,msg){
  ...
  if(!['ready','press','pick','leave','move','lobby'].includes(msg.type))fail('Acción desconocida.');

...
export function createRoomService({now=()=>performance.now(),seed=()=>randomBytes(4).readUInt32LE(),graceMs=10000,countdownMs=3000}={}){
 ...
 function begin(r){r.matchId=randomUUID();r.phase='countdown';r.startsAt=now()+countdownMs;r.result=null;r.engine=null;r.botAt=0;r.botPressId=0;r.players.forEach(p=>{p.ready=false;p.pressId=0;p.actions.clear();});publish(r);}

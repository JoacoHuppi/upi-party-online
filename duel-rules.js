...
function createTimedChoiceDuel(version,players,seed,{lanes,duration=30000,label}){
 const random=seededRandom(seed),state=new Map(players.map(id=>[id,{id,points:0,misses:0,blockedUntil:0,index:0,hits:0,mean:0,shownAt:0,stored:[0,0]}]));
 ...
 function snapshot(){return {rules:version,phase,winner,reason:phase==='over'?'score':null,remaining:Math.max(0,duration-elapsed),targets:players.map(id=>{const target=targets.get(id);return {...target,id,preview:version==='racket-duel-v1'?[...target.queue]:undefined,blocked:Math.max(0,(state.get(id).blockedUntil||0)-elapsed)};}),scores:players.map(id=>{const p=state.get(id);return {id,points:p.points,misses:p.misses,hits:p.hits,meanReaction:p.hits?Math.round(p.mean/p.hits):0,stored:[...p.stored]};})};}
 ...
 function choose(player,input,now){
  ...
  if(correct){p.hits++;p.points+=100;p.stored[input.lane]++;p.mean+=Math.max(0,elapsed-p.shownAt);p.shownAt=elapsed;}

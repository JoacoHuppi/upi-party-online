...
export const ODD_FAMILY_COUNT=12;
export function oddCellSignature(board,cell){return `${board.family}:${(board.rotation+(cell===board.odd?1:0))%4}`;}
export function createOddRound(seed){
 ...
 const next=()=>({index,odd:Math.floor(random()*ODD_RULES.cells),family:Math.floor(random()*ODD_FAMILY_COUNT),rotation:Math.floor(random()*4)});let board=next();
